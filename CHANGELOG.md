...
- **IPv6 SSRF fix** — private IPv6 addresses (`[::1]`, `fe80::`, `fc00::`, `::ffff:`) are now correctly blocked; previously they bypassed the SSRF filter due to how browsers normalise IPv6 hostnames
- **Crawl URL cap** — `crawlUrl` now returns at most 200 URLs per call, preventing unbounded crawls from being used as a timing attack
...
- PDF output uses `printBackground: true` and respects viewport dimensions when full-page is off
- `imageId` values now include the file extension (e.g. `abc123.jpg`) so storage is format-aware
...
- First published release of `@openkova/core` on npm
- Removed the `uuid` dependency in favour of the built-in `crypto.randomUUID()`
...
- Deployed via Docker on Railway with system Chromium
